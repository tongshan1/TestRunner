...
def main():
    ...
    cntl_q = context.Queue() # 控制信息传递队列
    data_q = context.Queue() # 具体数据传递队列
    exit_flag = context.Event() # 退出标记，初始值为False
    ...
    proc = Process(target=proc_proxy, args=(cntl_q, data_q, exit_flag))
    proc.start()
    ...
    while True:
        item = cntl_q.get()
        if item['event'] == 'data':
            proc = Process(target=proc_worker, args=(cntl_q, data_q))
            proc.start()
            proc_pool[proc.pid] = proc
